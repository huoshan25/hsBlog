...
<template>
  <div v-show="!articleEditorVisibility">
    <div class="form-container">
      <n-form ref="formRef" inline label-placement="left" label-align="left" :model="form" size="small">
        <div class="form-row">
          ...
          <n-form-item label="文章状态" path="status">
            <n-select w-100 v-model:value="form.status" :options="statusOption" clearable />
          </n-form-item>
          <n-form-item>
    ...
    <div m-b-15 flex>
      ...
      <n-button size="small" @click="newArticle('add')" dashed>
        <template #icon>
          <nuxt-img h-15 src="/svg/plusSign.svg" format="webp" />
        </template>
        新增文章
      </n-button>
    </div>
    ...
    <n-spin :show="loading">
      <template #description> 我知道你很急，但是你先别急。 </template>
      <n-data-table
        :columns="columns"
        :data="tableData"
