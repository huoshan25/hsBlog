...
// 根据 TypeScript 的推荐，如果你的代码中的异常对象类型无法确定，应该使用 ​unknown​ 类型。​unknown​ 类型比 ​any​ 类型更加安全和严格，它要求进行类型检查或类型断言才能访问其属性和方法。
// 并在处理异常时使用了类型断言 ​(err as Error).message​ 来访问异常的 ​message​ 属性。这样可以在保持类型安全的同时获取到异常对象的属性。
